...
The cube has six faces. Each face has four different vertices. Therefore the
cube has 24 vertices. Theoretically a cube only need eight vertices. In this
example each vertex has to be shared by three faces. Problem is, that each
vertex can only have one color, but what happens if we want that each of the six
faces has an another color? This is impossible if the cube is only defined with
eight vertices. There are more disadvantages if we only define the cube with
eight vertices, we talk later about. The only advantage with less vertices is
that we have to send less vertices to the graphic card, but in almost all
applications vertices are not a limiting factor. The memory consumption of
vertices in contrast to the memory consumption of textures is negligible. As
already written each vertex has one associated color. Besides the color there is
one UV coordinate for every vertex.
...
"""
One more thing about colors. When you look at the colors, defined inside the egg
file you may see that there are eight different colors defined. Black, white,
red, green, blue, yellow, purple and cyan. Each color is repeated three times.
When you run this sample you may ask yourself, why does this cube has thousands
of colors then? Who creates this nice gradients along the edges? We use a new
word here: Linear Interpolation. Today graphic cards are very good at linear
...
card does not know that a color consists of a R(ed), G(reen) and B(lue) part.
The graphic card interpolates each part of a color for itself. Here is an
...
specialized in parallel computing. This is also true for vertex shaders and
pixel shaders. Each calculation for a vertex or pixel is done individually. A
vertex never knows how his neighbor looks like and a pixel never knows what his
